...
if [ "$USER" != "root" ]; then
    ...
    cp -r /root/{.config,Desktop,.gtkrc-2.0} ${HOME}
...
chromeicon="/usr/share/applications/google-chrome.desktop"
cmd11="Exec\=\/usr\/bin\/google-chrome-stable\ %U"
cmd22="Exec\=\/usr\/bin\/google-chrome-stable\ %U\ --disable-gpu\ --user-data-dir\ --window-size=$VNC_RES_W,$VNC_RES_H\ --window-position=0,0"
sed -i "s/$cmd11/$cmd22/g" ${chromeicon}
